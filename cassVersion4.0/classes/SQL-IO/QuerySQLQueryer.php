...
class QuerySQLQueryer extends SQLQueryer{
	...
	function createQuery($name, $researchID, $xml=false, $locked=false){ //This is a function to create a new database row representing a research.
		$query= "INSERT INTO `tbl_query` (`query_id` ,`research_id` ,`xml_file` ,`locked` ,`name`,`visualize`)VALUES (NULL , '$researchID', '$xml', '$locked', '$name','false');";
		$result = $this->mdb2->query($query) or die('An unknown error occurred while updating the data');
		if(MDB2::isError($result)){
	...
	function updateQueryVisualization($id,$visualize){
		$query= "UPDATE `tbl_query` SET `visualize` = '$visualize'
				 WHERE `tbl_query`.`query_id` = '$id' LIMIT 1 ;";
		$result = $this->mdb2->query($query) or die('An unknown error occurred while updating the data');
	if(MDB2::isError($result)){
		return false;
	}else{
	...
	function updateQuery($id, $rid, $locked, $name, $xmlfile=null,$isVisualize){ //the xmlfile is assumed null as it's most likely not needed, but I'll leave it there just in case
		$query= "UPDATE `tbl_query` SET `research_id` = '$rid',
				`locked` = '$locked', `xml_file` = '$xmlfile',
				`name` = '$name',`visualize`= '$isVisualize' WHERE `tbl_query`.`query_id` = '$id' LIMIT 1 ;";
		$result = $this->mdb2->query($query) or die('An unknown error occurred while updating the data');
	if(MDB2::isError($result)){
		return false;
	}else{
