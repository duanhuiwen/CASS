...
class AnswerSQLQueryer extends SQLQueryer{
	...
	/*
	 * Used in content/visualize.php
	 * Display the graph with the results of users
	 * @var $qrid query id
	 * @return resource ID
	 */
	function visualizeQuery($qrid){
		$query = "SELECT `auth`.`username`, `a`.`time`, `qr`.`name`, `a`.`query_id`, `a`.`research_id`, `q`.`number`,`q`.`question`, CASE `q`.`question_type`
				WHEN 9 THEN (SELECT `anum`.`num` FROM `tbl_research` AS `r`, `tbl_query` AS `qr`, `tbl_question` AS `q`, `tbl_answer` AS `aid`, `tbl_num_answer` AS `anum` 
				WHERE `r`.`research_id`=`qr`.`research_id` AND `qr`.`query_id`=`q`.`query_id` AND `q`.`question_id`=`a`.`question_id` AND `anum`.`answer_id`=`a`.`answer_id` AND `aid`.`answer_id`=`a`.`answer_id`)
				END AS `answer`, `q`.`question_type` FROM `tbl_research` AS `r`, `tbl_query` AS `qr`, `tbl_question` AS `q`, `tbl_answer` AS `a`, `tbl_auth` AS `auth` 
				WHERE `q`.`question_type` = 9 AND `r`.`research_id`=`qr`.`research_id` AND `qr`.`visualize` = 1 AND /*Check if it's to be visualized*/ `qr`.`query_id`=`q`.`query_id` AND `q`.`question_id`=`a`.`question_id` AND
   						`a`.`UID`=`auth`.`UID` AND `qr`.`query_id`='$qrid' 
   				ORDER BY  `a`.`time`,`auth`.`username`,`q`.`number` ASC";
		$result = $this->mdb2->query($query) or die('An unknown error occurred while checking the data');
		if(MDB2::isError($result)){
			return "SelectError";
		}else{
